import { Vector3, Color, MathUtils as TMath } from "../../../libs/three.module.js";
...
function ParticleEmitterControl( options ) {
	...
	options.position = ShaderUtils.ensureTypedArg(
		options.position,
		ShaderUtils.types.OBJECT,
		{}
	);
	options.velocity = ShaderUtils.ensureTypedArg(
		options.velocity,
		ShaderUtils.types.OBJECT,
		{}
	);
	options.acceleration = ShaderUtils.ensureTypedArg(
		options.acceleration,
		ShaderUtils.types.OBJECT,
		{}
	);
	options.radius = ShaderUtils.ensureTypedArg(
		options.radius,
		ShaderUtils.types.OBJECT,
		{}
	);
	options.drag = ShaderUtils.ensureTypedArg(
		options.drag,
		ShaderUtils.types.OBJECT,
		{}
	);
	options.rotation = ShaderUtils.ensureTypedArg(
		options.rotation,
		ShaderUtils.types.OBJECT,
		{}
	);
	options.color = ShaderUtils.ensureTypedArg(
		options.color,
		ShaderUtils.types.OBJECT,
		{}
	);
	options.opacity = ShaderUtils.ensureTypedArg(
		options.opacity,
		ShaderUtils.types.OBJECT,
		{}
	);
	options.size = ShaderUtils.ensureTypedArg(
		options.size,
		ShaderUtils.types.OBJECT,
		{}
	);
	options.angle = ShaderUtils.ensureTypedArg(
		options.angle,
		ShaderUtils.types.OBJECT,
		{}
	);
	options.wiggle = ShaderUtils.ensureTypedArg(
		options.wiggle,
		ShaderUtils.types.OBJECT,
		{}
	);
	options.maxAge = ShaderUtils.ensureTypedArg(
		options.maxAge,
		ShaderUtils.types.OBJECT,
		{}
	);
	...
	if ( options.onParticleSpawn ) {
		...
		console.warn(
			"nunuStudio: onParticleSpawn has been removed. Please set properties directly to alter values at runtime."
		);
	...
	this.type = ShaderUtils.ensureTypedArg(
		options.type,
		ShaderUtils.types.NUMBER,
		ParticleDistributions.BOX
	);

	this.position = {
		_value: ShaderUtils.ensureInstanceOf(
			options.position.value,
			Vector3,
			new Vector3()
		),
		_spread: ShaderUtils.ensureInstanceOf(
			options.position.spread,
			Vector3,
			new Vector3()
		),
		_spreadClamp: ShaderUtils.ensureInstanceOf(
			options.position.spreadClamp,
			Vector3,
			new Vector3()
		),
		_distribution: ShaderUtils.ensureTypedArg(
			options.position.distribution,
			ShaderUtils.types.NUMBER,
			this.type
		),
		_randomise: ShaderUtils.ensureTypedArg(
			options.position.randomise,
			ShaderUtils.types.BOOLEAN,
			false
		),
		_radius: ShaderUtils.ensureTypedArg(
			options.position.radius,
			ShaderUtils.types.NUMBER,
			10
		),
		_radiusScale: ShaderUtils.ensureInstanceOf(
			options.position.radiusScale,
			Vector3,
			new Vector3( 1, 1, 1 )
		),
		_distributionClamp: ShaderUtils.ensureTypedArg(
	...
	this.drag = {
		_value: ShaderUtils.ensureTypedArg(
			options.drag.value,
			ShaderUtils.types.NUMBER,
			0
		),
		_spread: ShaderUtils.ensureTypedArg(
			options.drag.spread,
			ShaderUtils.types.NUMBER,
			0
		),
	...
	this.rotation = {
		_axis: ShaderUtils.ensureInstanceOf(
			options.rotation.axis,
			Vector3,
			new Vector3( 0.0, 1.0, 0.0 )
		),
		_axisSpread: ShaderUtils.ensureInstanceOf(
			options.rotation.axisSpread,
			Vector3,
			new Vector3()
		),
		_angle: ShaderUtils.ensureTypedArg(
			options.rotation.angle,
			ShaderUtils.types.NUMBER,
			0
		),
		_angleSpread: ShaderUtils.ensureTypedArg(
			options.rotation.angleSpread,
			ShaderUtils.types.NUMBER,
			0
		),
		_static: ShaderUtils.ensureTypedArg(
			options.rotation.static,
			ShaderUtils.types.BOOLEAN,
			false
		),
		_center: ShaderUtils.ensureInstanceOf(
	...
	this.color = {
		_value: ShaderUtils.ensureArrayInstanceOf(
			options.color.value,
			Color,
			new Color()
		),
		_spread: ShaderUtils.ensureArrayInstanceOf(
	...
	this.opacity = {
		_value: ShaderUtils.ensureArrayTypedArg(
			options.opacity.value,
			ShaderUtils.types.NUMBER,
			1
		),
		_spread: ShaderUtils.ensureArrayTypedArg(
	...
	this.particleCount = ShaderUtils.ensureTypedArg(
		options.particleCount,
		ShaderUtils.types.NUMBER,
		100
	);
	this.duration = ShaderUtils.ensureTypedArg(
		options.duration,
		ShaderUtils.types.NUMBER,
		null
	);
	this.isStatic = ShaderUtils.ensureTypedArg(
		options.isStatic,
		ShaderUtils.types.BOOLEAN,
		false
	);
	this.activeMultiplier = ShaderUtils.ensureTypedArg(
		options.activeMultiplier,
		ShaderUtils.types.NUMBER,
		1
	);
	this.direction = ShaderUtils.ensureTypedArg(
		options.direction,
		ShaderUtils.types.NUMBER,
		1
	);
	...
	this.alive = ShaderUtils.ensureTypedArg(
		options.alive,
		ShaderUtils.types.BOOLEAN,
		true
	);
	...
	this.resetFlags = {
      ...
      ShaderUtils.ensureTypedArg(
		...
		velocity: ShaderUtils.ensureTypedArg(
      ...
      ShaderUtils.ensureTypedArg(
		...
		rotation: ShaderUtils.ensureTypedArg(
			options.rotation.randomise,
			ShaderUtils.types.BOOLEAN,
			false
		),
		rotationCenter: ShaderUtils.ensureTypedArg(
			options.rotation.randomise,
			ShaderUtils.types.BOOLEAN,
			false
		),
		size: ShaderUtils.ensureTypedArg(
			options.size.randomise,
			ShaderUtils.types.BOOLEAN,
			false
		),
		color: ShaderUtils.ensureTypedArg(
			options.color.randomise,
			ShaderUtils.types.BOOLEAN,
			false
		),
		opacity: ShaderUtils.ensureTypedArg(
			options.opacity.randomise,
			ShaderUtils.types.BOOLEAN,
			false
		),
		angle: ShaderUtils.ensureTypedArg(
	...
	ShaderUtils.ensureValueOverLifetimeCompliance(
		this.color,
		ParticleEmitter.valueOverLifetimeLength,
		ParticleEmitter.valueOverLifetimeLength
	);
	ShaderUtils.ensureValueOverLifetimeCompliance(
		this.opacity,
...
ParticleEmitterControl.prototype._createGetterSetters = function (
	propObj,
	propName
) {
...
ParticleEmitterControl.prototype._assignPositionValue = function ( index ) {
	...
	switch ( distribution ) {
		...
		case ParticleDistributions.SPHERE:
			ShaderUtils.randomVector3OnSphere(
				attr,
				index,
				value,
				prop._radius,
		...
		case ParticleDistributions.DISC:
			ShaderUtils.randomVector3OnDisc(
				attr,
				index,
				value,
				prop._radius,
...
ParticleEmitterControl.prototype._assignForceValue = function (
	index,
	attrName
) {
	...
	if ( attrName === "acceleration" ) {
		...
		var drag = ShaderUtils.clamp(
			ShaderUtils.randomFloat( this.drag._value, this.drag._spread ),
			0,
			1
		);
...
ParticleEmitterControl.prototype._assignAbsLifetimeValue = function (
	index,
	propName
) {
	...
	if (
		ShaderUtils.arrayValuesAreEqual( prop._value ) &&
    ShaderUtils.arrayValuesAreEqual( prop._spread )
	) {
...
ParticleEmitterControl.prototype._assignRotationValue = function ( index ) {
	...
	this.attributes.rotation.typedArray.setVec3Components(
		index,
		ShaderUtils.getPackedRotationAxis(
			this.rotation._axis,
...
ParticleEmitterControl.prototype._updateAttributeUpdateRange = function (
	attr,
	i
) {
...
ParticleEmitterControl.prototype._checkParticleAges = function (
	start,
	end,
	params,
	dt
...
ParticleEmitterControl.prototype._activateParticles = function (
	activationStart,
	activationEnd,
	params,
	dtPerParticle
...
ParticleEmitterControl.prototype.remove = function () {
	...
	if ( this.group !== null ) {
		...
		console.error(
			"nunuStudio: ParticleEmitterControl does not belong to a group, cannot remove."
		);
