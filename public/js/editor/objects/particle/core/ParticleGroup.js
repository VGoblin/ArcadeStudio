...
function ParticleGroup( options ) {
	...
	options.texture = ShaderUtils.ensureTypedArg(
		options.texture,
	...
	this.fixedTimeStep = ShaderUtils.ensureTypedArg(
	...
	this.texture = ShaderUtils.ensureInstanceOf(
		options.texture.value,
		Texture,
		null
	);
	this.textureFrames = ShaderUtils.ensureInstanceOf(
		options.texture.frames,
		Vector2,
		new Vector2( 1, 1 )
	);
	this.textureFrameCount = ShaderUtils.ensureTypedArg(
		options.texture.frameCount,
		ShaderUtils.types.NUMBER,
		this.textureFrames.x * this.textureFrames.y
	);
	this.textureLoop = ShaderUtils.ensureTypedArg(
	...
	this.hasPerspective = ShaderUtils.ensureTypedArg(
		options.hasPerspective,
		ShaderUtils.types.BOOLEAN,
		true
	);
	this.colorize = ShaderUtils.ensureTypedArg(
		options.colorize,
		ShaderUtils.types.BOOLEAN,
		true
	);
	this.maxParticleCount = ShaderUtils.ensureTypedArg(
	...
	this.blending = ShaderUtils.ensureTypedArg(
		options.blending,
		ShaderUtils.types.NUMBER,
		AdditiveBlending
	);
	this.transparent = ShaderUtils.ensureTypedArg(
		options.transparent,
		ShaderUtils.types.BOOLEAN,
		true
	);
	this.alphaTest = parseFloat(
		ShaderUtils.ensureTypedArg( options.alphaTest, ShaderUtils.types.NUMBER, 0.0 )
	);
	this.depthWrite = ShaderUtils.ensureTypedArg(
		options.depthWrite,
		ShaderUtils.types.BOOLEAN,
		false
	);
	this.depthTest = ShaderUtils.ensureTypedArg(
		options.depthTest,
		ShaderUtils.types.BOOLEAN,
		true
	);
	this.fog = ShaderUtils.ensureTypedArg(
		options.fog,
		ShaderUtils.types.BOOLEAN,
		true
	);
	this.scale = ShaderUtils.ensureTypedArg(
		options.scale,
		ShaderUtils.types.NUMBER,
		300
	);
   ...
   */
	...
	if ( this.maxParticleCount === null ) {
		...
		console.warn(
			"nunuStudio: No maxParticleCount specified. Adding emitters after rendering will probably cause errors."
		);
...
ParticleGroup.prototype._updateDefines = function () {
	...
	for ( var i = emitters.length - 1; i >= 0; -- i ) {
		...
		if ( ! defines.SHOULD_CALCULATE_SPRITE ) {
			...
			defines.SHOULD_ROTATE_TEXTURE = defines.SHOULD_ROTATE_TEXTURE || Boolean (
				Math.max(
		...
		defines.SHOULD_ROTATE_PARTICLES = defines.SHOULD_ROTATE_PARTICLES || Boolean (
			Math.max( emitter.rotation.angle, emitter.rotation.angleSpread )
		);

		defines.SHOULD_WIGGLE_PARTICLES = defines.SHOULD_WIGGLE_PARTICLES || Boolean (
...
ParticleGroup.prototype.addEmitter = function ( emitter ) {
	...
	if ( emitter instanceof ParticleEmitterControl === false ) {
		...
		console.error(
			"nunuStudio: emitter argument must be instance of ParticleEmitterControl.",
			emitter
		);
		return;
	...
	// If the emitter already exists as a member of this group, then stop here, we don't want to add it again.
		...
		console.error(
			"nunuStudio: ParticleEmitterControl already exists in this group."
		);
		return;
	...
	// And finally, if the emitter is a member of another group, don't add it to this group.
		...
		console.error(
			"nunuStudio: ParticleEmitterControl already belongs to another group."
		);
		return;
	...
	if ( this.maxParticleCount !== null && this.particleCount > this.maxParticleCount ) {
		...
		console.warn(
			"arcade.studio: ParticleGroup maxParticleCount exceeded. Requesting",
			this.particleCount,
...
ParticleGroup.prototype.removeEmitter = function ( emitter ) {
	...
	if ( emitter instanceof ParticleEmitterControl === false ) {
		...
		console.error(
			"nunuStudio: emitter argument must be instance of ParticleEmitterControl. Was provided with:",
			emitter
		);
		return;
	...
	// Issue an error if the emitter isn't a member of this group.
		...
		console.error(
			"nunuStudio: ParticleEmitterControl does not exist in this group. Will not remove."
		);
		return;
...
ParticleGroup.prototype.releaseIntoPool = function ( emitter ) {
	...
	if ( emitter instanceof ParticleEmitterControl === false ) {
		...
		console.error(
			"nunuStudio: Argument is not instanceof ParticleEmitterControl:",
			emitter
		);
		return;
...
ParticleGroup.prototype._triggerSingleEmitter = function ( pos ) {
	...
	if ( emitter === null ) {
		...
		console.log( "arcade.studio: ParticleGroup pool ran out." );
		return;
...
ParticleGroup.prototype.toJSON = function ( meta ) {
	...
	data.texture = {};
	data.texture.value = this.textureId;
	if (this.texture == null)
		data.texture.value = this.textureId;
	else
		data.texture.value = this.texture.source.data.src;
