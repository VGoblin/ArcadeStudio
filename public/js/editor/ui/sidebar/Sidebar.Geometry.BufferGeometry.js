import { UIRow, UIText, UISpan, UIBreak } from '../components/ui.js';
...
function SidebarGeometryBufferGeometry( editor ) {
	...
	function update( object ) {
		...
		if ( geometry && geometry.isBufferGeometry ) {
			...
			var text = new UIText( strings.getKey( 'sidebar/geometry/buffer_geometry/attributes' ) );
			container.add( text );
			...
			if ( index !== null ) {
				...
				container2.add( new UIText( ( index.count ).format() ) );
			...
			for ( var name in attributes ) {
				...
				container2.add( new UIText( ( attribute.count ).format() + ' (' + attribute.itemSize + ')' ) );
				container2.add( new UIBreak() );
