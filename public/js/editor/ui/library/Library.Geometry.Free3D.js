...
function LibraryGeometryFree3D( editor, json ) {
	...
	function buildFree3D( json, container ) {
		...
		for ( var name in accordionChildren ) {
			...
			buildFree3D( accordionChildren[ name ], accordion.body );
		}
	...
	buildFree3D( json, panel );

	container.add( new LibraryComponentBannerButton( editor, panel, json.thumbUrl, strings.getKey( 'library/geometry/free3D' ) ) );
	container.add( panel );
