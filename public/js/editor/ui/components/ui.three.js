...
function UINewTexture( editor, mapping ) {
	...
	canvas.style.border = '1px solid #7292db';
		...
		UtilsHelper.chooseSingleFile( function ( files ) {
			...
			Promise.all( editor.loader.loadFiles( files, null, 'Image') ).then( function ( results ) {
				...
				for ( let i = 0; i < files.length; i++ ) {
					...
					asset.id = files[i].id;
					asset.imageId = files[i].imageId;
					loading.style.display = "none";

					scope.setValue( texture );
					if ( scope.onChangeCallback ) scope.onChangeCallback( -1 );
	...
	canvas.addEventListener( 'drop', function ( e ) {
		console.log("UI New Texture drop called");
		...
		if ( extension === 'hdr' ) {
			...
			reader.addEventListener( 'load', function ( event ) {
				...
				image.addEventListener( 'load', function () {
					...
					texture.sourceFile = file.name;
					// RGBFormat is removed from three.js. RGBAFormat is the default value
...
//UIEnvTexture
function UIEnvTexture( editor, mapping ) {
		...
		UtilsHelper.chooseSingleFile( function ( files ) {
			...
			Promise.all( editor.loader.loadFiles( files, null, 'Image') ).then( function ( results ) {
				...
				for ( let i = 0; i < files.length; i++ ) {
					...
					scope.setValue( texture );
					console.log("files[i].id: ",files[i].id);
					if ( scope.onChangeCallback ) scope.onChangeCallback( -1 );
...
UIEnvTexture.prototype = Object.create( UIElement.prototype );
UIEnvTexture.prototype.constructor = UIEnvTexture;

UIEnvTexture.prototype.getValue = function () {

	return this.texture;

};

UIEnvTexture.prototype.setValue = function ( texture ) {
	var canvas = this.dom.children[ 0 ];
	var context = canvas.getContext( '2d' );
...
UIEnvTexture.prototype.setEncoding = function ( encoding ) {
...
UIEnvTexture.prototype.onChange = function ( callback ) {
...
//UIEnvTexture
function UIEnvEquirect( editor, mapping ) {
	...
	canvas.addEventListener( 'drop', function ( e ) {
		...
		if ( assetType == 'Environment' ) {
			...
			if (typeof asset == 'undefined') {
				...
				asset.texture ? scope.setValue( asset.texture ) : asset.apply(true);
...
UIEnvEquirect.prototype = Object.create( UIElement.prototype );
UIEnvEquirect.prototype.constructor = UIEnvEquirect;

UIEnvEquirect.prototype.getValue = function () {

	return this.texture;

};

UIEnvEquirect.prototype.setValue = function ( texture ) {
	var canvas = this.dom.children[ 0 ];
	var context = canvas.getContext( '2d' );
...
UIEnvEquirect.prototype.setEncoding = function ( encoding ) {
...
UIEnvEquirect.prototype.onChange = function ( callback ) {
...
UIOutliner.prototype.setOptions = function ( options ) {
	...
	function onDblClick() {
		...
		name.addEventListener( 'keyup', function() {
			// prevent keyup event listeners from being called when typing names for the element
	...
	function onDragLeave() {
		...
		if (this.className.indexOf("no-effect") != -1)
			this.className = 'option no-effect';	
		else
	...
	function onDrop( event ) {
		...
		var cl = () => {
			...
			if ( area < 0.25 ) {
				...
				{
					moveObject( object, scene, null, event.shiftKey );
				}
				...
				if ( this.nextSibling !== null ) {
					...
					nextObject = scene.getObjectById( this.value );
					var cnt = scene.children.indexOf(nextObject);
					if (cnt == scene.children.length - 1)
					{
						nextObject = null;
					...
					{
						nextObject = scene.children[cnt + 1];
						parent = nextObject.parent;
					}
...
export function renderToCanvas( texture ) {
...
export { UITexture, UICubeTexture, UIOutliner, UIPoints, UIPoints2, UIPoints3, UIBoolean, UINewTexture, UIEnvTexture, UIEnvEquirect };