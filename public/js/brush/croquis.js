...
function Croquis(mainCanvas, opts, convertToImg, imageDataList, properties) {
    ...
    function pushDirtyRectUndo(x, y, width, height, index) {
        ...
        x = _min(w, _max(0, x));
        y = _min(h, _max(0, y));
        width = _min(w, _max(x, right)) - x;
        height = _min(h, _max(y, bottom)) - y;
        if ((x % 1) > 0)
            ++width;
        if ((y % 1) > 0)
            ++height;
        x = x | 0;
        y = y | 0;
        width = _min(w - x, Math.ceil(width));
        height = _min(h - y, Math.ceil(height));
        if ((width == 0) || (height == 0)) {
    ...
    */
    ...
    self.down = function (x, y, pressure, azimuthAngle) {
        ...
        var down = () => { tool.down(x, y, pressure, azimuthAngle) };
...
Croquis.Stabilizer = function (down, move, up, level, weight,
    ...
    interval = interval || 5;
    var follow = 1 - _min(0.95, _max(0, weight));
    var paramTable = [];
...
var HSLToRGB = (h, s, l) => {
    ...
    l /= 100;
    const k = n => (n + h / 30) % 12;
    const a = s * _min(l, 1 - l);
    const f = n =>
      l - a * _max(-1, _min(k(n) - 3, _min(9 - k(n), 1)));
...
var HSBToRGB = (h, s, b) => {
    ...
    b /= 100;
    const k = (n) => (n + h / 60) % 6;
    const f = (n) => b * (1 - s * _max(0, _min(k(n), 4 - k(n), 1)));
    return [255 * f(5), 255 * f(3), 255 * f(1)];
};

var darker = function(r, g,b, alpha) {
    return [_max((int)(r * FACTOR), 0),
         _max((int)(g * FACTOR), 0),
         _max((int)(b * FACTOR), 0),
         getAlpha()];
}
...
var brighter = function(r,g, b, alpha, FACTOR) {
    ...
    return [_min((int)(r/FACTOR), 255),
         _min((int)(g/FACTOR), 255),
         _min((int)(b/FACTOR), 255),
         alpha];
}
//
...
Croquis.Brush = function () {
    ...
    this.setColor = function (value = '#000', second = '#fff') {
        firstcolor = value;
        if (secondarycolor != second)
        {
            secondarycolor = second;
            this.setEffectImage(orgeffectimage);
        }
    ...
    var d3_effect = false;
    this.getD3Effect = function () {
        return d3_effect;
    }
    this.setD3Effect = function (value) {
        d3_effect = value;
    }
    var normalSpread = 0;
    ...
    //outline attributes
    var outlineSize = 0;
    this.getOutlineSize = function () {
        return outlineSize;
    }
    this.setOutlineSize = function (value) {
        outlineSize = value;
    }
    var outlineContour = 1;
    this.getOutlineContour = function () {
        return outlineContour;
    }
    this.setOutlineContour = function (value) {
        outlineContour = value;
    }
    var outlinePlacement = 1;
    this.getOutlinePlacement = function () {
        return outlinePlacement;
    }
    this.setOutlinePlacement = function (value) {
        outlinePlacement = value;
    }
    //shape attributes
    var shapeScatter = 0;
    ...
    var dynamicPressureSize = 0;
    this.getDynamicPressureSize = function () {
        return dynamicPressureSize;
    }
    this.setDynamicPressureSize = function (value) {
        dynamicPressureSize = value;
    }
    var dynamicPressureOpacity = 0;
    this.getDynamicPressureOpacity = function () {
        return dynamicPressureOpacity;
    }
    this.setDynamicPressureOpacity = function (value) {
        dynamicPressureOpacity = value;
    }
    ...
    var bsoftness = 0;
    this.getSoftness = function () {
        return bsoftness;
    }
    this.setSoftness = function (value) {
        bsoftness = value;
    }
    var flow = 1;
    ...
    var effectimage = null, orgeffectimage = null;
    var imageRatio = 1;
    ...
    this.setEffectImage = function (value) {
        let scolor = fabric.util.colorValues(secondarycolor);
        if (value == null) {
            effectimage = null;
        }
        else {
            orgeffectimage = effectimage = value;
            try
            {
                var tempCanvas = new OffscreenCanvas(effectimage.width, effectimage.height);
                const tempContext = tempCanvas.getContext("2d");
                tempContext.drawImage(effectimage, 0, 0);
                let effectimageData = tempContext.getImageData(0, 0, tempCanvas.width, tempCanvas.height);
                var pixelData = effectimageData.data;
                for (var i = 0; i < pixelData.length; i += 4) {
                    pixelData[i + 3] = (pixelData[i] - 105) * 1.7 * (0.2 + spacing * 0.8);
                    pixelData[i] = scolor[0];
                    pixelData[i + 1] = scolor[1];
                    pixelData[i + 2] = scolor[2];
                }
                tempContext.clearRect(0, 0, tempCanvas.width, tempCanvas.height);
                tempContext.putImageData(effectimageData, 0, 0);
                effectimage = tempCanvas;
            }
            catch(e)
            {
                var tempCanvas = document.createElement('canvas');
                tempCanvas.width = effectimage.width;
                tempCanvas.height = effectimage.height;
                const tempContext = tempCanvas.getContext("2d", { willReadFrequently: true });
                tempContext.drawImage(effectimage, 0, 0);
                var effectimageData = tempContext.getImageData(0, 0, tempCanvas.width, tempCanvas.height);
                var pixelData = effectimageData.data;
                for (var i = 0; i < pixelData.length; i += 4) {
                     pixelData[i + 3] = (pixelData[i] - 105) * 1.7 * (0.2 + spacing * 0.8);
                     pixelData[i] = scolor[0];
    ...
    function drawReserved() {
        if (reserved != null) {
            drawTo(reserved.x, reserved.y, reserved.scale, 0, 0, flow);
            reserved = null;
        }
    }
    ...
    function drawTo(x, y, scale, pressure, azimuthAngle, gOpacity) {
        ...
        for (let i = 0; i < cnt; i ++)
        {
            ...
            drawImage(x, y, width, pressure, gOpacity);
            context.restore();
        }
    ...
    function drawImage(x, y, size, pressure, gOpacity) {
        if (transformedImageIsDirty)
            transformImage(x, y, size, pressure, gOpacity);
        try {
            context.filter = `blur(${bsoftness * bsize * 0.5}px)`;
    ...
    function transformImage(x, y, size, pressure, gOpacity) {
        let fcolor = color;
        let scolor = fabric.util.colorValues(secondarycolor);
        if (outlineSize > 0)
        {
        ...
        if (outlineSize > 0 && !isFrame)
        {
            ...
            context.drawImage(transformedImage, 0, 0, transformedImage.width, transformedImage.height, -size * (outlineSize * outlinePlacement), -size * imageRatio * (outlineSize * outlinePlacement), size * (1 + outlineSize), size * imageRatio * (1 + outlineSize));
        }
        //////////////////////////
        if (grainEffect == 2 && patternCol.indexOf("none") == -1)
        {
        ...
        brushContext.fillStyle = 'rgba(' + fcolor[0] + ',' + fcolor[1] + ',' + fcolor[2] + ',' + 1 + ')';
        brushContext.fillRect(0, 0, transformedImage.width, transformedImage.height);
    }
    ...
    this.down = function(x, y, pressure, azimuthAngle) {
        ...
        //////////////////////
        spCurScale = spPrevScale = 1 + _min(dynamicSpeedSize, 0);
        spCurOpacity = spPrevOpacity = 1 + _min(dynamicSpeedOpacity, 0);
        let pressure_opacity = (1 + _min(dynamicPressureOpacity, 0)) - pressure * dynamicPressureOpacity;
        if (scale > 0) {
            if (normalSpread != 0 || tangentSpread != 0)
                reserved = {x: x, y: y, scale: scale};
            else
            {
                context.globalAlpha = flow * (spPrevOpacity == 0 ? spPrevOpacity : pressure_opacity);
                drawTo(x, y, scale, pressure, azimuthAngle, flow);
            }
        }
        delta = 0;
        lastX = prevX = x;
    ...
    this.move = function(x, y, pressure, azimuthAngle) {
        var scale = pressure;
        var pressure_opacity = 0;
        if (pressure == 0)
            scale = 1;
        else
        {
            scale = Math.min(1, (1.2 + _min(dynamicPressureSize * 1.2, 0)) - pressure * dynamicPressureSize * 1.2);
            pressure_opacity = (1 + _min(dynamicPressureOpacity, 0)) - pressure * dynamicPressureOpacity;
        }
        if (context == null)
            throw 'brush needs the context';
        if (scale <= 0) {
        ...
        if (dynamicSpeedSize != 0) {
            spCurScale = (1 + _min(dynamicSpeedSize, 0)) - (speed * 0.1 * dynamicSpeedSize);
            spCurScale = _min(1, _max(0.1, spCurScale));
        ...
        if (dynamicSpeedOpacity != 0) {
            spCurOpacity = (1 + _min(dynamicSpeedOpacity, 0)) - (speed * 0.1 * dynamicSpeedOpacity);
            spCurOpacity = _min(1, _max(0.1, spCurOpacity));
        ...
        if (ld < drawSpacing) {
            lastX = x;
            lastY = y;
            drawTo(lastX, lastY, (pressure == 0 ? spCurScale : scale), pressure, azimuthAngle, flow);
            delta -= drawSpacing;
        } else {
            while(delta >= drawSpacing) {
                ...
                context.globalAlpha = opacity * (pressure == 0 ? spPrevOpacity : pressure_opacity);
                if (flow > 0)
                {
                    drawTo(lastX, lastY, (pressure == 0 ? spPrevScale : prevScale), pressure, azimuthAngle, opacity);
