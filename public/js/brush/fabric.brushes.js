			...
			if (this.canvas.getObjects().length != 0) {
				...
				if (layer.find(".layer-hide")[0].style.display != 'none') {
					...
					if (selectedGroup.length > 0) {
						...
						img.filters.push(new fabric.Image.filters.Brightness({ brightness: parseFloat(/*selectedGroup.brightness ? selectedGroup.brightness : */0) }));
						img.filters.push(new fabric.Image.filters.Contrast({ contrast: parseFloat(/*selectedGroup.contrast ? selectedGroup.contrast : */0) }));
						img.filters.push(new fabric.Image.filters.Saturation({ saturation: parseFloat(/*selectedGroup.saturation ? selectedGroup.saturation : */0) }));
						img.filters.push(new fabric.Image.filters.HueRotation({ rotation: parseFloat(/*selectedGroup.hue ? selectedGroup.hue : */0) }));
						img.filters.push(new fabric.Image.filters.Blur({ blur: parseFloat(/*selectedGroup.blur ? selectedGroup.blur : */0) }));
						img.applyFilters();

						selectedGroup.addWithUpdate(img);
						this.canvas.add(img);
						this.canvas.remove(img);
					}
				}
				else {
					//alert("You can not draw because the selected layer is hidden for now.");
				}
			}
		}
		else {
